...
function custom_scripts() {
  wp_enqueue_style('roots_custom_css', get_template_directory_uri() . '/assets/css/app.css');
  wp_enqueue_style('roots_custom_font', get_template_directory_uri() . '/assets/css/font-awesome.css');
}
add_action('wp_enqueue_scripts', 'custom_scripts', 200);

// Registering New Menu Placeholders
register_nav_menus( array(
	'top_navigation' => 'Top Navigation',
	'footer_navigation' => 'Footer Navigation'
) );

// Adding Options page for ACF
...
// Removing p tags from ACF WSYSIWIG Editor
//remove_filter ('acf_the_content', 'wpautop');

add_action( 'after_setup_theme', 'new_featured_image' );
function new_featured_image() {
  add_image_size( 'product-large', 318, 288, true ); // (cropped)
  add_image_size( 'product-thumb', 58, 58, true ); // (cropped)
}
...
function catch_that_image() {
  ...
  if(empty($first_img)) {
    $first_img = get_bloginfo ( 'stylesheet_directory' ) . '/assets/img/logo-darwins.png';
  }
  return $first_img;
}
