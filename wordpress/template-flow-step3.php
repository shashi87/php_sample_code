			...
			?>
                  ...
                  <div class="editable count input_edit">
                    <select class="selectship" id="shippingfreq_<?php echo $pid; ?>" style="margin-bottom: 5px;width: 100px;">
                      ...
                      <?php for($j = 0; $j<=50 ;$j++){
					if($j==$weeks){ $slected = "selected"; } else {  $slected = ""; }
					?>
                  ...
                  <?php $_SESSION["shipping_weeks"]=$weeks?>
                  <div class="editable count input_view"  id="weeks_<?php echo $pid; ?>"><?php echo $weeks; ?> weeks supply</div>
	  ...
	  $total =  $price + $_SESSION["shipment_price"];
		  ...
		  ?>
			...
			//echo '$'.number_format($priceperweek, 2, '.', '').'/week'; ?>
              ...
              <?php $i++; } ?>
	       <div style="border-top:1px solid #cfcfcf;padding-top:5px;text-align:right;" class="order-rightnew">Total:&nbsp;<?php echo '$'.number_format($grandtotal, 2, '.', ''); ?>
              <?php if(isset($_SESSION) && $_SESSION["order_species"] == 'multiple'){?>
              <div class="row"><div class="col-lg-12"><span class="shipspan">
                <div class="editable count input_edit">
                  <select class="selectship" id="shippingfreq_<?php echo $pid; ?>" style="margin-bottom: 5px;width: 100px;">
                    <option value="">Select Weeks</option>
                    <?php for($j = 0; $j<=50 ;$j++){
...
<script>
	...
	$('.items').each(function(){
		...
		var petfood = petfood + parseFloat($(this).find('[id^="pet_round_weight_"]').html()) ;
	...
	$(".dogUpdateBtn").on("click",function(){
		...
		$.ajax({
			...
			success: function (results) {
				...
				if(data.pet_type == 0){
					$("#recurring_order_price_1").html("$"+addshipping);
					...
					$("#recurring_order_price_2").html("$"+addshipping);
					var perweekprice = parseFloat(addshipping/data.petData[0].weeks).toFixed(2);
